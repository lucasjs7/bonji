<x-app-layout>
    <x-slot name="header">
        <x-admin-title-page>Mídias de {{ $product->name }}</x-admin-title-page>
    </x-slot>
    <x-body>
        <div class="col-span-8">
            <x-button bg="blue-600" href="{{ route('admin.products.media.create', $product->id) }}">
                <x-slot name="svg">
        ...
        <div class="col-span-4">
            <form action="{{ route('admin.products.media.index', $product->id) }}" method="get" class="flex flex-row">
                ...
                <x-crud.table>
                    <x-crud.thead>
                        <x-crud.tr>
                            ...
                            <x-crud.th></x-crud.th>
                            <x-crud.th>Caminho</x-crud.th>
                            <x-crud.th>Tipo</x-crud.th>
                            <x-crud.th>Tamanho (bytes)</x-crud.th>
                            <x-crud.th></x-crud.th>
                        </x-crud.tr>
                    </x-crud.thead>
                    <x-crud.tbody>
                        @forelse($media as $md)
                            <x-crud.tr>
                                <x-crud.td>{{ $md->order }}</x-crud.td>
                                <x-crud.td>
                                    <img src="{{ url('storage/' . $md->thumbnail) }}" class="max-w-[32px]">
                                </x-crud.td>
                                <x-crud.td>{{ $md->file }}</x-crud.td>
                                <x-crud.td>{{ $md->extension }}</x-crud.td>
                                <x-crud.td>{{ $md->size }}</x-crud.td>
                                <x-crud.td class="flex flex-wrap items-center justify-end">
                                    ...
                                    <x-button href="{{ route('admin.products.media.edit', [$product->id, $md->id]) }}" class="px-0" pattern="clear" bg="slate-400 ml-1">
                                        <x-slot name="svg">
                                    ...
                                    <form action="{{ route('admin.products.media.destroy', [$product->id, $md->id]) }}" method="post">
                                        @method('DELETE')
                                        @csrf
                            ...
                            <x-crud.tr>
                                <x-crud.td class="text-center !pt-8 !pb-6" colspan="6">
                                    ...
                                    @if(!empty(request()->search))
                                        <x-button href="{{ route('admin.products.media.index', $product->id) }}" bg="blue-600" size="small" pattern="hollow" class="mt-2">
                                            <x-slot name="svg">
